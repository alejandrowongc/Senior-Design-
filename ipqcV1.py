...
motor = RpiMotorLib.A4988Nema(DIR_PIN, STEP_PIN, (21,21,21), "DRV8825")
...
def motor_to_active():
    global MOTOR_ACTIVE
    if not MOTOR_ACTIVE:
        motor.motor_go(False, "Full", STEPS, STEP_DELAY, False, 0.05)
        MOTOR_ACTIVE = True
...
def motor_to_neutral():
    global MOTOR_ACTIVE
    if MOTOR_ACTIVE:
        motor.motor_go(True, "Full", STEPS, STEP_DELAY, False, 0.05)
        MOTOR_ACTIVE = False
...
while True:
    ...
    # Undistort the frame correctly at 800x480
    ...
    decision = None  # final PASS or REJECT outcome for this frame
    ...
    for cnt in contours:
        ...
        # Draw measurement bounding box and text
        ...
        if center_band_left <= cx <= center_band_right:
            ...
            cv2.putText(roi, result, (x, y+h+20),
